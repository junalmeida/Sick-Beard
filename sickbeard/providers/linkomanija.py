...
class LinkomanijaProvider(generic.TorrentProvider):
    ###################################################################################################
    def __init__(self):
        generic.TorrentProvider.__init__(self, "Linkomanija")
        self.supportsBacklog = True
        self.cache = LinkomanijaCache(self)     
        self.url = 'http://www.linkomanija.net/'
        self.name = "Linkomanija"
        self.session = None
    ...
    def isEnabled(self):
        return sickbeard.LINKOMANIJA
    
    ###################################################################################################
    
    def imageName(self):
        return 'linkomanija.png'
    ...
    def _doSearch(self, search_params, show=None):
        logger.log("[Linkomanija] Performing Search: {0}".format(search_params))
        searchUrl = self.url + "browse.php?c30=1&c60=1&incldead=0&search=" + urllib.quote(search_params)
    ...
    def parseResults(self, searchUrl):
        ...
        results = []
        if data:
            for torrent in re.compile('<td align="left"><a href="details\?.*?"><b>(?P<title>.*?)</b></a><a class="index" href="download\.php(?P<url>.*?)"><img style="border:none; float:right;" alt="download"',re.MULTILINE|re.DOTALL).finditer(data):
    ...
    def _doLogin(self):
        login_params  = {
            'username': sickbeard.LINKOMANIJA_USERNAME,
            'password': sickbeard.LINKOMANIJA_PASSWORD,
            'login': 'submit'
        }
...
class LinkomanijaCache(tvcache.TVCache):
    ...
    def _getRSSData(self):
        xml = ''
        if  sickbeard.LINKOMANIJA_RSSHASH:
            self.rss_url = provider.url + "rss.php?feed=dl&cat[]=30&cat[]=60&passkey={0}".format(sickbeard.LINKOMANIJA_RSSHASH)
            ...
            url = provider.url + "browse.php?c30=1&c60=1&incldead=0&search="
