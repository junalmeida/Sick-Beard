...
# URL: https://github.com/medariox/Sick-Beard
# 
# Based on the awesome provider script work by Jodi Jones <venom@gen-x.co.nz>
# Thanks fly out to D. for helping out.
#
...
class DemonoidProvider(generic.TorrentProvider):
    ...
    def _doSearch(self, search_params, show=None):
        ...
        searchUrl = self.url + "files/?category=3&subcategory=All&language=0&quality=All&seeded=2&external=2&query=" + urllib.quote(search_params)
    ...
    def parseResults(self, searchUrl):
        ...
        if data:
            ...
            for torrent in re.compile('_pad"><a href="/files/details/(?P<url>\d+)/\d+/"\s?>(?:&quot;|")?(?P<title>.*?)(?:&quot;|")?</a>',re.MULTILINE|re.DOTALL).finditer(data):
                item = (torrent.group('title').replace('.',' '), self.url + "files/download/" + torrent.group('url'))
                results.append(item)                        
    ...
    def _doLogin(self):
        login_params  = {
            'nickname': sickbeard.DEMONOID_USERNAME,
            'password': sickbeard.DEMONOID_PASSWORD
        }
        ...
        try:
            response = self.session.post(self.url + "account_handler.php", data=login_params, timeout=30, verify=False)
        except (requests.exceptions.ConnectionError, requests.exceptions.HTTPError), e:
            raise Exception("[" + self.name + "] _doLogin() Error: " + ex(e))
            return False
        
        if re.search("Erroneous password or validation code|<title>User log in - Demonoid</title>",response.text):
